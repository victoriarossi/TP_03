...
void InitDrive(drive_t *drive, int base_cmd, int base_ctl, int irq, int ack, int hook, int drive_num)
{
	drive->status		= 0;
	drive->w_status		= 0;
...
int FlushCache(void)
{
	...
	if (CommandSimple(&cmd) == OK) // CommandSimple is CommandOut and WaitForBusyErrorWriteFaultCleared():
	{
		//kprintf("Flush!");
	}
	else
...
int CommandSimple(struct command_t *cmd)
{
	int result;
	drive_t *drive_ptr = &driveT[0];
